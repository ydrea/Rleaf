    ...
    const res = await fetch(
      `${process.env.REACT_APP_SERVER}/novi`,
      {
        ...
        body: JSON.stringify({
          ...
          autor: exif.Artist ?? '',
          copyright: decodeString(exif.Copyright ?? exif.rights?.value ?? exif.CopyrightNotice ?? ''),
          tagovi: (Array.isArray(exif.subject) ? exif.subject : []).join(','),
          lon: `${exif.longitude ?? ''}`,
          lat: `${exif.latitude ?? ''}`,
          naziv: decodeString(exif.Title ?? exif.title?.value ?? exif.ObjectName ?? ''),
          naziv_eng: '',
          opis: decodeString(exif.ImageDescription ?? exif.description?.value ?? exif.Caption ?? ''),
          opis_eng: '',
          kategorija: '',
          copyright_holder: '',
          doi: '',
